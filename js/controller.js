...
control.directive('iframeOnload', [function(){
return {
    scope: {
    ...
    link: function(scope, element, attrs){
        element.on('load', function(){
          successurl = this.contentWindow.location;
...
control.controller('cartCtrl',function($scope,$state,$ionicLoading,$ionicModal,$http, $ionicPopup, $sce){
                   ...
                   $ionicModal.fromTemplateUrl('templates/stripe.html',{
                                               scope: $scope,
                                               animation: 'slide-in-up'
                                               }).then(function(modal) {
...
//                                     alert(JSON.stringify(data));
                   ...
                   $scope.success = function(){
                    ...
                    if(successurl=='http://getappco.de/shop_app/stripe/done.html'){
                                       $scope.stripeModal.hide();
                   ...
                   // var ref = window.open(url, 'popup','width=640','height=480','menubar=no,location=yes');
                   // $(ref).on('closed', function() {
                   //  alert('hi');
                   //  alert(this.location);
                   //                      if(this.location=='http://getappco.de/shop_app/stripe/done.html/'){
                   //                      ref.close();
                   ...
                   ref.addEventListener('loadstop', function(event) {
                                        ...
                                        ref.close();
                                        var res = url.replace(/&/g, "^");
                                        console.log('http://getappco.de/shop_app/stripe/cart_details.php?url='+res);
                                        $scope.payment_success('http://getappco.de/shop_app/stripe/cart_details.php?url='+res);
                                        }
                                        });
                   }
                   ...
                   .success(function(data, status){
                            ...
                            .success(function(data, status){
                                     ...
                                     formData1.append('price','Pending');
                                     formData1.append('additional_field',order_bid);
...
//                                     console.log(JSON.stringify(contentntarr));
                   ...
                   $scope.editseo = function(){
                   
                   $ionicLoading.show({
                                      template: '<i class="icon ion-loading-a"></i>&nbsp;Please wait...'
                                      });
                   $http({method: "PUT", url:"http://build.myappbuilder.com/api/elements/meta-tags.json", cache: false, params:{"api_key":appkey,"id":elementid,"permalink":$scope.add.pl,"seo_title":$scope.add.st,"meta_keywords":$scope.add.mk,"meta_description":$scope.add.md}})
                   .success(function(data, status){
                   
                   $scope.add.pl = data.permalink;
                   $scope.add.st = data.seo_title;
                   $scope.add.mk = data.meta_keywords;
                   $scope.add.md = data.meta_description;
                   $ionicLoading.hide();
                   $state.reload();
...
function readURL1(input) {
                   ...
                   $scope.add.pl = contentntarr.permalink;
                   $scope.add.st = contentntarr.seo_title;
                   $scope.add.mk = contentntarr.meta_keywords;
                   $scope.add.md = contentntarr.meta_description;
